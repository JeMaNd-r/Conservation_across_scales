...
# - change raw data files to load alternative dataset: prepare protection and 
#   prepare soil data
...
if(!dir.exists(paste0(here::here(), "/results/sensitivity_globalAlternativeDataset"))){
  dir.create(paste0(here::here(), "/results/sensitivity_globalAlternativeDataset"))
}
...
data_glob <- readxl::read_xlsx(paste0(here::here(), "/data_raw/GlobalAtlasv2_conservation_heterogeneity_papers_v2.xlsx"),
                               sheet = "Data")
data_glob <- data_glob %>% filter(!is.na("ID_microbes"))
...
table(data_pa$PA) #0 = 248, 1 = 231

write_csv(data_pa, paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/PA_assignment_global_atlas.csv"))
...
coord_glob <- readxl::read_xlsx(paste0(here::here(), "/data_raw/GlobalAtlasv2_conservation_heterogeneity_papers_v2.xlsx"),
                                sheet = "Data")
coord_glob #383 
coord_glob %>% filter(!is.na(ID_microbes))#383 

data_glob <- read_csv(here::here("data_raw/Diversity_global_atlas.csv")) #472 

sort(unique(data_glob$SampleID)) #472
sort(unique(coord_glob$ID_microbes)) #383

setdiff(unique(data_glob$SampleID), unique(coord_glob$ID_microbes))
setdiff(unique(coord_glob$ID_microbes), unique(data_glob$SampleID))
...
data_glob <- coord_glob %>%
  dplyr::select("Order_ID", "ID_microbes", "Latitude_c", "Longitude_c", 
                ...
                "Nutrient_service") %>%
  mutate(ID_numb = str_replace(ID_microbes,"FM", "1")) %>% #ID_microbes T goes to 221, FMxxx to 269 -> not over 1000
  mutate(ID_numb = str_replace(ID_numb, "T", "2")) %>%
  mutate(ID_numb = as.numeric(ID_numb)) %>%
  filter(!is.na(ID_numb)) %>%
  full_join(data_glob %>%
              mutate(ID_numb = str_replace(SampleID, "FM", "1")) %>%
              mutate(ID_numb = str_replace(ID_numb, "T0", "T")) %>% # some 0 that are not present in coord_glob
              mutate(ID_numb = str_replace(ID_numb, "T0", "T")) %>%
              mutate(ID_numb = str_replace(ID_numb, "T", "2")) %>%
              mutate(ID_numb = as.numeric(ID_numb)), 
            by = "ID_numb") %>%
...
protect_glob <- read_csv(paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/PA_assignment_global_atlas.csv"))

data_glob <- f_add_protect(data = data_glob, 
                           data_pa = protect_glob %>%
                             mutate(ID_numb = str_replace(ID_microbes,"FM", "1")) %>% #ID_microbes T goes to 221, FMxxx to 269
                             mutate(ID_numb = str_replace(ID_numb, "T", "2")) %>%
                             mutate(ID_numb = as.numeric(ID_numb)), 
                           col_id="ID_numb")
rm(protect_glob)

nrow(data_glob %>% filter(PA==1)) #80
nrow(data_glob %>% filter(!is.na(PA_type))) #135
nrow(data_glob %>% filter(PA==0)) #303
...
data_glob[data_glob$LC=="Moss_heath" & !is.na(data_glob$LC), "LC"] <- "Other"
unique(data_glob$LC)

table(data_glob$LC) # G 97, Moss 3, S 145, W 138 
...
data_glob <- data_glob[complete.cases(data_glob[,c(mahal_vars, fns, "LC")]),] #377 (98.4%)   
data_glob
...
#- - - - - - - - - - - - - - - - - - - - - - - - - - - -
### Save ####
write_csv(data_glob, file=paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/Data_clean_global.csv"))
...
# Save
write.csv(list_colinear$env_vif, file=paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/VIF_envVars_global.csv"), row.names = F)
write.csv(list_colinear$corMatSpearman, paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/corMatSpearman_envVars_global.csv"), row.names = F)
write.csv(list_colinear$corMatPearson, paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/corMatPearson_envVars_global.csv"), row.names = F)
...
data_clean <- read_csv(paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/Data_clean_", temp_scale, ".csv"))
data_clean

## Explore data
summary(as.factor(data_clean$PA)) #G: 297 nonPA and 80 PAs
# number of observations (raw)
nrow(data_clean); nrow(data_clean[data_clean$PA,])  #G: 377 with 80 PAs
...
f_check_pairs <- function(data, col_id, col_lc, vars_z){
  ...
  write.csv(unpaired_pa, file=paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/Unpaired_protected_sites_", Sys.Date(), ".csv"), row.names = F)
...
count_nonPA <- count_nonPA[[1]] #G: 17 <7 & <5, 18 <10 (SampleID 1225 with No_nonPA = 7)
#head(count_nonPA)
...
nrow(data_clean); nrow(data_clean[data_clean$PA,]) #G: nrow=377 with 80 PAs
...
nrow(data_clean); nrow(data_clean[data_clean$PA,]) #nrow=360 with 63 PAs
...
# e.g. global drylands: only 3 unprotected on 0 protected for Other & only 1 PA for Woodland & 0 for Cropland
# global alternative dataset: 
...
#save(pa_pairs, file=paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/Pairs_paNonpa_1000trails_", Sys.Date(),".RData"))
write_csv(pa_pairs, file=paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/Pairs_paNonpa_1000trails_", Sys.Date(),".csv"))
...
#- - - - - - - - - - - - - - - - - - - - - - - - - - - -
### Load PA-nonPA pairs ####
pa_pairs <- read_csv(file=paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/Pairs_paNonpa_1000trails_", temp_date,".csv"))
head(pa_pairs)
...
save(d_list,  file=paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/d_1000_trails_", temp_scale, ".RData"))
...
save(pars_list, file=paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/pars_PAtypes_Bayesian_", temp_scale, ".RData"))
...
load(file=paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/pars_PAtypes_Bayesian_", temp_scale, ".RData")) #pars_list
...
save(pars_sample, file=paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/pars_PAtypes_Bayesian_df_", temp_scale, ".RData"))

#- - - - - - - - - - - - - - - - - - - - - - - - - - - -
### Calculate contrast between protection types ####
# 
# load(file=paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/pars_PAtypes_Bayesian_df_", temp_scale, ".RData")) #pars_sample
# 
...
save(pred_list, file=paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/PAranks_Bayesian_", temp_scale, ".RData"))
pred_sample <- pred_list %>% group_by(LC) %>% slice_sample(n = 10000)

save(pred_sample, file=paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/PAranks_Bayesian_", temp_scale, "_sample10k.RData"))

save(fixed_effects, file=paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/PAranks_Bayesian_", temp_scale, "_summary.RData"))
sink(paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/PAranks_Bayesian_", temp_scale, ".txt"))
fixed_effects
sink()

# extract emtrends
load(file=paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/PAranks_Bayesian_", temp_scale, "_summary.RData")) #fixed_effects
...
write_csv(emtrends, file=paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/PAranks_Bayesian_", temp_scale, "_emtrends.csv"))
...
# load effect sizes
load(file=paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/d_1000_trails_", temp_scale, ".RData")) #d_list
...
ggsave(filename=paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/Data_locations_", temp_scale,".png"),
       plot = last_plot())
...
ggplot() +
  ...
  theme_minimal()
ggsave(filename=paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/Locations_connection_", temp_scale, ".pdf"),
       plot = last_plot())
...
ggsave(filename=paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/Data_boxplot_mahal.distance_", temp_scale, ".png"),
       plot = last_plot())
...
# ggsave(filename=paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/Data_violin_d-value_global.png"),
#        plot = last_plot())
...
sink(file=paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/D-values_", temp_scale, ".txt"))
cat("#################################################", sep="\n")
...
ggsave(filename=paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/Results_boxplot_estimates_", temp_scale, ".png"),
       plot = last_plot())
...
pred_list <- rbind(get(load(paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/PAranks_Bayesian_global_sample10k.RData"))), 
...
#- - - - - - - - - - - - - - - - - - - - - - - - - - - -
pars_glob <- read_csv(file=paste0(here::here(), "/results/sensitivity_globalAlternativeDataset/PAranks_Bayesian_global_emtrends.csv"))
