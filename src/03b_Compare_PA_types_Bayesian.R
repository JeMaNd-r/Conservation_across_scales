...
for(temp_scale in c("global")){ # ,"continental", "regional"
  ...
  for(temp_fns in fns){
    ...
    # priors
    prior_list[[temp_fns]][["intercept"]] <- brms::prior_summary(output_intercept)
    prior_list[[temp_fns]][["slope"]] <- brms::prior_summary(output_slope)
  ...
  save(prior_list, file=paste0(here::here(), "/intermediates/PAranks_Priors_", temp_scale, ".RData"))
...
for(temp_scale in c("continental", "regional")){
  ...
  save(prior_list, file=paste0(here::here(), "/intermediates/PAranks_PriorsWithRandomFactor_", temp_scale, ".RData"))
