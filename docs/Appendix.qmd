...
Note: The effect of protection status and types on soil biodiversity and functioning was also tested at global scale but datasets available were not suitable for full comparison. Dataset used for global comparison is described in [Appendix 4](#appendix-4-effect-of-protection-at-global-scale). Results at global scale are shown in supplementary figures and tables but should be taken with caution.
...
#| label: Figure 2.1
#| fig-cap: "Appendix Figure 2.1: Workflow to assess the effect of conservation measures on soil biodiversity and functioning across three spatial scales. Analysis steps of estimation of effect size (1) and random-slope models (2). Results for the random-slope models are given in Appendix 3. Icons: soil: Romy Zeiss (CC-BY 4.0), all others: PowerPoint."
...
Although the pairing of environmentally similar sites is minimizing differences between protected and unprotected sites (see Appendix Fig. 2.2), we still expected an effect of the environment on soil biodiversity and functioning. We found contradicting results regarding the effect of the environment when testing for the correlation between Mahalanobis distance and environmental variables (Appendix Figure 2.2-2.3 and Appendix Table 2.1), as well as the correlation between spatial distance and pairwise difference in attribute values (Appendix Figure 2.4-2.6 and Appendix Table 2.2). We found significant effects of protection status on soil biodiversity and functioning attributes despite nonsignificant correlations between Mahalanobis distance and environmental variables (Appendix Figure 2.2-2.3 and Appendix Table 2.1), indicating that the effects found cannot fully be explained by Mahalanobis distance (i.e. environmental distinctness). We did found significant correlations between spatial distance and the absolute difference in soil biodiversity and functioning attributes of protected-unprotected pairs (`r corr_n_significant` out of `r corr_n_total`), however, `r corr_n_negative` of these correlations were even negative, meaning that the difference between attributes in protected and unprotected soils got smaller with increasing distance (Appendix Figure 2.4-2.6 and Appendix Table 2.2). This indicates that the significant effects cannot be fully explained by differences in environmental conditions and spatial distance of paired samples alone but may in fact be related to the protection status.

```{r, echo=FALSE, warning=FALSE, message=FALSE}
#| label: Figure 2.2
#| fig-cap: "Appendix Figure 2.2: Mahalanobis distance between protected and unprotected sites is smaller in pairs than across all investigates sites. Distance is shown on y-axis at different scales without (bottom) and with proper pairing (top) of environmentally similar sites (i.e. pairing based on Mahalanobis threshold). This shows that paired sites are indeed more environmentally similar than compared with pairing of all sites without Mahalanobis threshold."
...
knitr::kable(corr_mahal,
             caption = "Appendix Table 2.1: Correlation of the Mahalanobis distance between paired protected and unprotected sites and individual difference in their soil biodiversity and functioning attributes averaged across habitat types and spatial scales. Correlation was calculated as Spearman correlation coefficient using cor.test() function. Negative correlation coefficients show the correlation between the always non-negative Mahalanobis distance between paired sites and lower attributes in protected sites than in unprotected ones, that is the larger the Mahalanobis distance, the lower the soil biodiversity and functioning attributes in protected sites compared with unprotected sites. Positive correlation coefficients show vice versa higher attributes in protected sites with increasing Mahalanobis distance. Asterisks highlight significant correlations, that is p-values ≤ 0.05; p-values for the correlation test are approximate because of the natural presence of tied values in the data, which result from the randomization process. ns - not significant.")
```
...
knitr::kable(corr_dist,
             ...
             caption = "Appendix Table 2.2: Correlation of the spatial distance between paired global protected and unprotected sites (x-axis) and individual absolute difference in their soil biodiversity and functioning attributes (y-axis). Correlation was calculated as Spearman correlation coefficient using cor.test() function. Values in paranthesis are p-values and test statistic; p-values for the correlation test are approximate because of the natural presence of tied values in the data, which result from the randomization process.")
```

```{r, echo=FALSE, warning=FALSE, message=FALSE}
#| label: Figure 2.4
#| fig-cap: "Appendix Figure 2.4: Correlation of the spatial distance between paired global protected and unprotected sites (x-axis) and individual absolute difference in their soil biodiversity and functioning attributes (y-axis). Correlation was calculated as Spearman correlation coefficient using cor.test() function."
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
#| label: Figure 2.5
#| fig-cap: "Appendix Figure 2.5: Correlation of the spatial distance between paired continental protected and unprotected sites (x-axis) and individual absolute difference in their soil biodiversity and functioning attributes (y-axis). Correlation was calculated as Spearman correlation coefficient using cor.test() function."
...
Results for the predicted intercept and slope estimates per spatial scale and habitat type can be found in Appendix Figure 3.3 to 3.8. It is again important to note that sample size for some protection types is rather low and results especially for the random-intercept but also random-slope model are less meaningful than effect size estimates for protected-unprotected pairs.
...
## Appendix 4: Effect of protection at global scale {#appendix-4-effect-of-protection-at-global-scale}

To analyze the effect of protection status and types on soil biodiversity and functioning, data were retrieved from an updated version of the dataset presented in [@maestre2015; @maestre2022a; @maestre2022b]. The dataset includes 551 locations collected in drylands across six continents, which are defined as “areas with an aridity index (precipitation divided by potential evapotranspiration) \<0.65” [@maestre2022a; @maestre2022b]. In the updated version of the dataset, 18 sites were added to the already published sampling sites from the BIOCOM [@maestre2015] and BIODESERT project [@maestre2022a; @maestre2022b]. Sampling collection and analysis follow protocols previously described [@maestre2015; @maestre2022a; @maestre2022b]. To our knowledge, this is the best available dataset at time that covers one type of ecosystem very well to compare soil biodiversity globally. The limitation to one biome type, namely drylands, allows for meaningful comparisons of similar ecosystems. The dataset contained 248 sites with complete biodiversity data of all taxonomic groups investigated, but 330 sites with complete bacterial diversity data. We therefore decided to run two analyses for bacteria: one with all 330 sites and one with only 248 sites. Results for the complete bacterial diversity data can be found in [Appendix 4.2](#appendix-4-2-sensitivity-analysis-for-global-bacterial-data).
...
### Appendix 4.1: Results of global-scale analysis

Results of the analysis of protection status at global scale were presented in the figures and tables above. Here, a visualization of the effect size estimates is added.

```{r, echo=FALSE, warning=FALSE, message=FALSE}
#| label: Figure 4.1
#| fig-cap: "Appendix Figure 4.1: Effect size estimates for soil biodiversity and functioning attributes averaged across 1000 randomized pairings in global drylands. Effect size magnitudes were calculated as Cohen’s D. Statistically non-significant effect sizes are defined as 95% confidence intervals (CI) crossing 0. Number of samples was 39 for global scale for each habitat type and attribute."

knitr::include_graphics(here::here("figures", "Results_d-value_medianCI_global.png"))

```

### Appendix 4.2: Sensitivity analysis for global bacterial data
...
#| layout-ncol: 2
#| fig-cap:
#|   - "Appendix Figure 4.3: Attribute values of (a) all-complete dataset and"
...
#| fig-cap:
#|   - "Appendix Figure 4.4: Effect size estimates averaged over 1,000 randomizations for all-complete (red) and bacteria-complete (blue) global dataset. Thick and thin bars indicate 66 % and 95 % confidence intervals. Points represent median effect sizes. Absolute effect sizes are regarded as small (>0.2), medium (>0.5) and large (>0.8) indicated by vertical lines."
...
In contrast to the patterns from the all-complete dataset, we found a positive effect of protection on bacterial richness and Shannon index in the bacteria-complete dataset, while effect size for dissimilarity was not significantly different from 0 (Appendix Table 4.1, Appendix Figure 4.4); the all-complete dataset showed the exact opposite pattern with less diverse and similar bacteria communities in protected areas. Higher richness and Shannon index in protected areas in the bacteria-complete dataset are also exactly opposite to the pattern we found at regional scale, indicating again that effects are not consistent across scales. However, we still found the mixed pattern of the effect of protection status on soil biodiversity, here specifically for bacteria, with no consistent direction but being positive and negative for different diversity aspects (i.e. richness, Shannon index, dissimilarity).

#### Random-intercept model

Random-intercept models did not differ for the bacteria-complete and all-complete dataset regarding those protected area categories that are present in both (Appendix Table 4.2; Appendix Figure 4.5). For the categories being only present in the bacteria-complete dataset, we found similarly low bacterial richness and Shannon index as well as similarly high dissimilarity for protected sites of category Ib and VI; both align most with richness trends for protected sites of category V and otherwise did not differ from attribute values in other IUCN categories.
